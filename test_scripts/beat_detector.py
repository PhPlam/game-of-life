# source: https://github.com/maxemitchell/beat-detection-python
...
def beat_detect(in_data):
    ...
    # Frequency Ranges for each important test_audio group
    ...
    sub_bass = np.max(audio_fft[sub_bass_indices],initial=0.1)
    bass = np.max(audio_fft[bass_indices],initial=0.1)
    low_midrange = np.max(audio_fft[low_midrange_indices],initial=0.1)
    midrange = np.max(audio_fft[midrange_indices],initial=0.1)
    upper_midrange = np.max(audio_fft[upper_midrange_indices],initial=0.1)
    presence = np.max(audio_fft[presence_indices],initial=0.1)
    brilliance = np.max(audio_fft[brilliance_indices],initial=0.1)
